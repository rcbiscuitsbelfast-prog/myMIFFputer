...
interface WindowManagerProps {
  ...
  onSizeChange: (instanceId: string, size: { width: number; height: number }) => void;
}
    ...
    <div className="window-manager">
      {windows.map((win) => (
        <Rnd
          ...
          minWidth={win.windowOptions?.minWidth ?? 320}
          minHeight={win.windowOptions?.minHeight ?? 220}
          bounds="parent"
          dragHandleClassName="desktop-window__chrome"
          onDragStart={() => onFocus(win.instanceId)}
          onDragStop={(_, data) => onPositionChange(win.instanceId, { x: data.x, y: data.y })}
          onResizeStart={() => onFocus(win.instanceId)}
          ...
          enableResizing={win.windowOptions?.resizable ?? true}
          className={clsx('desktop-window', {
          ...
          <article className="desktop-window__inner" onMouseDownCapture={() => onFocus(win.instanceId)}>
            <header className="desktop-window__chrome" onMouseDown={() => onFocus(win.instanceId)}>
              <div className="desktop-window__title">
                {win.icon ? (
                  win.icon.includes('/') ? (
                    <img src={win.icon} alt={`${win.title} icon`} />
                  ) : (
