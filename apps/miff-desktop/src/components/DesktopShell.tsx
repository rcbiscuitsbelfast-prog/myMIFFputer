...
function getWallpaperStyle(wallpaper?: { image?: string; tint?: string }, fallbackTint?: string): CSSProperties {
...
function createWindowFromModule(module: MiffModule, offset: number, nextZ: number): DesktopWindowInstance {
  return {
    instanceId: nanoid(8),
    moduleId: module.id,
    title: module.title,
...
function getMaxZ(windows: DesktopWindowInstance[]) {
...
export function DesktopShell() {
  ...
  const [windows, setWindows] = useState<DesktopWindowInstance[]>([]);
  ...
  useEffect(() => {
    ...
    if (theme.accent) root.style.setProperty('--miff-accent', theme.accent);
    if (theme.panel) root.style.setProperty('--miff-panel', theme.panel);
    if (theme.text) root.style.setProperty('--miff-text-primary', theme.text);
    if (theme.mutedText) root.style.setProperty('--miff-text-muted', theme.mutedText);
    if (theme.taskbar) root.style.setProperty('--miff-taskbar', theme.taskbar);
    if (theme.wallpaperTint) root.style.setProperty('--miff-wallpaper-tint', theme.wallpaperTint);
    if (theme.background) root.style.setProperty('--miff-backdrop', theme.background);
  }, [content]);
  ...
  const focusWindow = useCallback((instanceId: string) => {
    setWindows((prev) => {
      ...
      return prev.map((win) =>
        win.instanceId === instanceId ? { ...win, minimized: false, zIndex: maxZ + 1 } : win
      );
    });
  ...
  const updatePosition = useCallback((instanceId: string, position: { x: number; y: number }) => {
    setWindows((prev) => prev.map((win) => (win.instanceId === instanceId ? { ...win, position } : win)));
  }, []);

  const updateSize = useCallback((instanceId: string, size: { width: number; height: number }) => {
    ...
    (moduleId: string) => {
      ...
      setWindows((prev) => {
        ...
        if (existing) {
          ...
          return prev.map((win) =>
            win.instanceId === existing.instanceId
  ...
  const toggleModuleFromTaskbar = useCallback(
    (moduleId: string) => {
  ...
  return (
    <div className="desktop-shell">
      ...
      <div className="desktop-shell__workspace">
        ...
        {error && (
          <div className="desktop-shell__status" style={{ marginTop: '2rem' }}>
