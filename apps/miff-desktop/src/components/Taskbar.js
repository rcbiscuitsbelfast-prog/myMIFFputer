    ...
    return (<footer className="taskbar">
      ...
      <div className="taskbar__items">
        {modules.map((module) => {
            ...
            return (<button key={module.id} type="button" className={clsx('taskbar__item', {
                    'taskbar__item--open': Boolean(openWindow),
                    'taskbar__item--active': isActive,
                })} onClick={() => onModuleToggle(module.id)} title={module.title} aria-label={module.title}>
              {isImageIcon ? (<img src={module.icon} alt={module.title}/>) : (<span>{module.icon ?? module.title.slice(0, 2)}</span>)}
            </button>);
        })}
      </div>
